...
export class APIStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const predictionsTable = new dynamodb.Table(
      this,
      "itwasntamanualPredictions",
      {
    ...
    const userVotesTable = new dynamodb.Table(
      this,
      "itwasntamanualUserPredictionVotes",
      {
        billingMode: dynamodb.BillingMode.PAY_PER_REQUEST,
        removalPolicy: RemovalPolicy.DESTROY,
        partitionKey: {
    ...
    const backingCertificate = new acm.Certificate(
      this,
      "backingapiitwasntamanualcomCertificate",
      {
    ...
    new route53.ARecord(this, "BackingApi53", {
      ...
      target: RecordTarget.fromAlias(
        new ApiGatewayv2DomainProperties(
          backingDomainName.regionalDomainName,
          backingDomainName.regionalHostedZoneId
        )
      ),
    });
    ...
    lambdas.forEach(({ handler, lambdaFile, endpointPath, methods }) => {
      const lambdaFunction = new NodejsFunction(
        this,
        "itwasntamanual" + methods.join() + endpointPath,
        {
      ...
      httpApi.addRoutes({
        path: endpointPath,
        methods: methods,
        integration: new HttpLambdaIntegration(endpointPath, lambdaFunction),
      });
    });
    ...
    const certificate = new acm.Certificate(
      this,
      "apiitwasntamanualcomCertificate",
      {
        domainName,
        validation: acm.CertificateValidation.fromDns(zone),
      }
    ...
    new cdk.CfnOutput(this, "ApiCloudfrontDistributionId", {
